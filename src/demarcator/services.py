...
class DemarcatorService:
    def __init__(self, store: SQLiteRepository):
        self.store = store

    def list_connectors(self) -> list[dict[str, Any]]:
        return [connector.to_dict() for connector in self.store.list_connectors()]

    def list_workflows(self) -> list[dict[str, Any]]:
        return [workflow.to_dict() for workflow in self.store.list_workflows()]

    def list_rules(self) -> list[dict[str, Any]]:
        return [rule.to_dict() for rule in self.store.list_rules()]

    def list_people(self) -> list[dict[str, Any]]:
        grants = self.store.list_workflow_grants()
        return [
            {
                **user.to_dict(),
                "workflow_ids": sorted(grant.workflow_id for grant in grants if grant.user_id == user.user_id),
            }
    ...
    def list_approvals(self) -> list[dict[str, Any]]:
        return [approval.to_dict() for approval in self.store.list_approvals()]

    def list_audit_events(self) -> list[dict[str, Any]]:
        return [event.to_dict() for event in self.store.list_audit_events()]

    def list_alerts(self) -> list[dict[str, Any]]:
        return [alert.to_dict() for alert in self.store.list_alerts()]

    def summary(self) -> dict[str, Any]:
        approvals = self.store.list_approvals()
        connectors = self.store.list_connectors()
        workflows = self.store.list_workflows()
        recent_runs = self.store.list_runs()
        alerts = self.store.list_alerts()
        pending_approvals = sum(1 for approval in approvals if approval.status == ApprovalStatus.PENDING)
        failed_connectors = sum(1 for connector in connectors if connector.health == ConnectorHealth.FAILED)
        ...
        return {
            "connected_sources": len(connectors),
    ...
    def run_workflow(self, request: RunRequest) -> WorkflowRun:
        ...
        with self.store.transaction():
            if not allowed_sources:
                ...
                run.output_summary = "Workflow run blocked because no requested sources were allowed."
                self._record_alert(
            ...
            if request.requested_action and allowed_sources:
                ...
                run.action_outcome = action_outcome
                run.status = status
                run.approval_id = approval_id
                run.output_summary = output_summary
            elif request.requested_action:
                run.action_outcome = "blocked"
                run.status = RunStatus.BLOCKED
                run.output_summary = "Requested action blocked because the workflow had no allowed sources."
    ...
    def _evaluate_sources(
        ...
        rules = self.store.list_rules_for_workflow(workflow_id)
        allowed_sources: list[RequestedSource] = []
        blocked_sources: list[RequestedSource] = []
        for source in requested_sources:
            matching_rules = [rule for rule in rules if rule.connector_id == source.connector_id]
    ...
    def _ensure_actor_can_run(self, actor: User, workflow: Workflow) -> None:
        if actor.role == Role.ADMIN:
            return
        allowed = self.store.has_workflow_grant(actor.user_id, workflow.workflow_id)
        if not allowed:
